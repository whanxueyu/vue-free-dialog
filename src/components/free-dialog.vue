...
function initThumbnail() {
  ...
  if (thumbnailRef.value) {
    ...
    if (thOp?.right) {
      ...
      thumbnailRef.value.style.left = "";
    } else if (thOp?.left !== undefined && thOp?.left !== null) {
      thumbnailRef.value.style.left = autoUnit(thOp.left);
      thumbnailRef.value.style.right = "";
    } else {
      // 默认出现在右侧
      thumbnailRef.value.style.right = "10px";
      thumbnailRef.value.style.left = "";
    }

    if (thOp?.bottom) {
      thumbnailRef.value.style.bottom = autoUnit(thOp.bottom);
      thumbnailRef.value.style.top = "";
    } else if (thOp?.top !== undefined && thOp?.top !== null) {
      thumbnailRef.value.style.top = autoUnit(thOp.top);
      thumbnailRef.value.style.bottom = "";
    } else {
      // 默认出现在顶部
      thumbnailRef.value.style.top = "100px";
      thumbnailRef.value.style.bottom = "";
    }
...
function initSize() {
  ...
  if (
    isAllowValue(mergeProps.value.left) &&
    isAllowValue(mergeProps.value.right)
  ) {
    w =
      warpperEle.offsetWidth -
      (mergeProps.value.left as number) -
      (mergeProps.value.right as number);
  ...
  // 如果同时存在 left right 则自动计算
  if (
    isAllowValue(mergeProps.value.top) &&
    isAllowValue(mergeProps.value.bottom)
  ) {
    h =
      warpperEle.offsetHeight -
      (mergeProps.value.top as number) -
      (mergeProps.value.bottom as number);
